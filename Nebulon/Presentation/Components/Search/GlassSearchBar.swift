...
/// iOS-style search field: magnifier, clear button, and a round Cancel button
/// that slides in while editing. Liquid Glass on iOS 26+, material below.
struct GlassSearchBar: View {
    ...
    var body: some View {
        // Neighboring glass surfaces must share one GlassEffectContainer —
        // without it their backdrop layers overlap and swallow each other's
        // touches, which made the cancel button unresponsive
        if #available(iOS 26, *) {
...
private struct GlassSearchBarBackground: ViewModifier {
    func body(content: Content) -> some View {
        if #available(iOS 26, *) {
            // Plain .regular — the field is not a tap target, so it must not
            // claim interactive glass touches next to the cancel button
            content
...
private struct GlassCircleBackground: ViewModifier {
    func body(content: Content) -> some View {
        if #available(iOS 26, *) {
            // Interactive is safe here — the shared GlassEffectContainer
            // keeps neighboring glass layers from stealing touches
            content
